...
productApp.get("/products-name/:name", (req, res) => {    // handle GET request to fetch products by name
    // log url params for debugging
    console.log("req.params", req.params)  // example: {name:tab}
    // read name from url parameter
    let productname = req.params.name
    // filter products matching the given name
    let product = products.filter(productObj => productObj.name === productname)
    console.log(product) // log filtered results
    // if no matches, return 404
    if (product.length === 0) {
        return res.status(404).json({ message: "products not found of this name" })
    }
    // return matching products
    res.status(200).json({ message: "products by name found", payload: product })
})
